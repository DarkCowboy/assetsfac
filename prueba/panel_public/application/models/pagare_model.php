...
class Pagare_Model extends Base_Model {
    ...
    public function pause_operacion($operaciones) {
        $id_recibo = $operaciones['id_recibo'];
        $fecha_pausado = $operaciones['fecha_pausado'];
        unset($operaciones['id_recibo']);
        unset($operaciones['fecha_pausado']);
        foreach ($operaciones as $filas) {
            $this->db->where('id_solicitud', $filas['id_solicitud']);
            $this->db->update('solicitudes', array('pause' => 1, 'id_recibo' => $id_recibo, 'fecha_pausado' => $fecha_pausado));
        }
    }
