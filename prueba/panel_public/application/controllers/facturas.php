...
//                                $factura[$i]['numero'] = $data[0];
                                $factura[$i]['numero_factura'] = $data[0];
                                $factura[$i]['deudor'] = $data[1];
                                $factura[$i]['rif'] = $data[2];
                                $factura[$i]['fecha_emision'] = str_replace('-', '/', $data[3]);
                                $factura[$i]['fecha_vencimiento'] = str_replace('-', '/', $data[4]);
                                $factura[$i]['valor_nominal'] = $data[5];
...
//                                $factura[$j]['numero'] = $data[0];
                                $factura[$j]['numero_factura'] = $data[0];
                                $factura[$j]['deudor'] = $data[1];
                                $factura[$j]['rif'] = $data[2];
                                $factura[$j]['fecha_emision'] = str_replace('-', '/', $data[3]);
                                $factura[$j]['fecha_vencimiento'] = str_replace('-', '/', $data[4]);
                                $factura[$j]['valor_nominal'] = $data[5];
        ...
        if ($valida == true) {
            if ($this->input->post()) {
                ...
                if (count($form) > 1) {
                    ...
                    $solicitud['monto_solicitud'] = $form['result_nominal_input'];
...
//                        $this->load->view('redirect', array('destino' => base_url() . 'facturas/panel_facturas'));
                        $this->load->view('redirect', array('destino' => base_url() . 'ver_planilla/'.$id_solicitud));
                    } else {
