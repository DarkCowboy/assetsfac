...
//       var fechareg = /^(\d{1,2})(\/)(\d{1,2})(\/)(\d{4})$/;
        var fechareg = /^(\d{1,2})(\/)(\d{1,2})(\/)(\d{6})$/;
        if (valTipo == 'fecha-545461')
        {
            var value = $(this).val();
            var fechaCompleta = value.match(fechareg);
            console.debug(value.match(fechareg));
            ...
            console.debug(fechaCompleta[1]);
            console.debug(fechaCompleta[3]);
            console.debug(fechaCompleta[5]);

            if (dia < 1 || dia > 31) {
                $(this).focus().addClass('error');
                    mostrarError();
                $('.msgErrorValidator').html("El valor del d�a debe estar comprendido entre 1 y 31.");
                error = 1;
                return false;
            }
            if (mes < 1 || mes > 12) {
                $(this).focus().addClass('error');
                    mostrarError();
                $('.msgErrorValidator').html("El valor del mes debe estar comprendido entre 1 y 12.");
                error = 1;
                return false;
            }
            if ((mes == 4 || mes == 6 || mes == 9 || mes == 11) && dia == 31) {
                $(this).focus().addClass('error');
                    mostrarError();
                $('.msgErrorValidator').html("El mes " + mes + " no tiene 31 d�as!");
                error = 1;
                return false;
            }
            if (mes == 2) { // bisiesto
                ...
                if (dia > 29 || (dia == 29 && !bisiesto)) {
                    $(this).focus().addClass('error');
                    mostrarError();
                    $('.msgErrorValidator').html("Febrero del " + anio + " no contiene " + dia + " dias!");
                    error = 1;
                    return false;
                }
            }
