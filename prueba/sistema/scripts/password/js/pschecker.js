(function ($) {
    $.fn.extend({
        pschecker: function (options) {
            var settings = $.extend({ minlength: 6, maxlength: 16, onPasswordValidate: null, onPasswordMatch: null }, options);
            return this.each(function () {
                ...
                password.keyup(validatePassword).blur(validatePassword).focus(validatePassword).load(validatePassword);
                cPassword.keyup(validatePassword).blur(validatePassword).focus(validatePassword).load(validatePassword);
...
//                        meter.removeClass('strong').removeClass('medium').removeClass('weak');
                        meter.removeClass('fuerte').removeClass('medio').removeClass('debil');
                    if (pstr.length > 0) {
                        var rx = new RegExp(/^(?=(.*[a-z]){1,})(?=(.*[\d]){1,})(?=(.*[\W]){1,})(?!.*\s).{7,30}$/);
                        if (rx.test(pstr)) {
                            meter.addClass('fuerte');
                            meter.html("Fuerte");
                        }
                        else {
                            ...
                            if (result > 2) {
                                meter.addClass('medio');
                                meter.html("Medio");
                            }
                            else {
                                meter.addClass('debil');
                                meter.html("Debil");
                            }
                        }
