...
class Elimina_Egreso extends MY_Controller {
    ...
    public function index($id_operacion) {
        if ($this->centinela) {
            ...
            $operacion_eliminar = $this->instrucciones_model->get_instrucciones_by_id($id_operacion);
            $this->bancos_model->update_corte($operacion_eliminar, 1);
            $this->instrucciones_model->eliminar_instruccion($id_operacion);
            $this->load->view('egresos/editado_correctamente');
        }
    }
