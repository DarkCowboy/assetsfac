...
class Facturas_Model extends Base_Model {
    ...
    public function aceptar_facturas($facturas) {
        foreach ($facturas as $row) {
            $row['liquidado_compra'] = str_replace('.', '', $row['liquidado_compra']);
            $row['liquidado_compra'] = str_replace('.', '', $row['liquidado_compra']);
            $row['liquidado_compra'] = str_replace(',', '.', $row['liquidado_compra']);
            $this->db->where('id_factura', $row['id_factura']);
            unset($row['id_factura']);
...
//        return $this->db->query("select * from facturas
//            left join recibos using(id_recibo)
//            where id_solicitud = {$id_solicitud}
//                ")->result_array();
        return $this->db->query("select * from facturas left OUTER join recibos using (id_recibo) where id_solicitud = {$id_solicitud}")->result_array();
//        return $this->db->get_where('facturas', array('id_solicitud' => $id_solicitud))->result_array();
    ...
    public function cancelar_facturas($id_factura) {
        $this->db->where('id_factura', $id_factura);
        $this->db->update('facturas', array('status' => '2'));
    }

    public function registrar_pago_factura($pago) {
