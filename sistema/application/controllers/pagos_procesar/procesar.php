    ...
    public function index($id_instruccion) {
        if ($this->centinela) {
            ...
            if ($this->input->post()) {
                ...
                $this->load->view('pagos_procesar/procesado_listo', $this->data);
            } else {

                switch ($this->data['instruccion']['t_instrumento']) {
                    case 'Cheque':
                        $this->load->view('pagos_procesar/proce_pago_cheq', $this->data);
                        break;
                    case 'Transferencia':
                        $this->load->view('pagos_procesar/proce_pago_trans', $this->data);
                        break;
                    case 'Efectivo':
                        $this->load->view('pagos_procesar/proce_pago_efectivo', $this->data);
                        break;
                }
            }
        }
