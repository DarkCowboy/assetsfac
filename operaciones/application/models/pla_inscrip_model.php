...
public function create_table_perfil_empresa() {
   ...
   public function create_table_inscripcion_pn() {
    ...
    public function salvar_planilla_inscripcion() {
        ...
        $r_e["rep_leg_reg_pj"] = $form["rep_leg_reg_pj"];
        // Datos Bancarios
        ...
        $verifica_email = $this->db->get_where('personas_juridicas', array('email_pj' => $form["email_pj"]))->row_array();
        ...
        if (count($verifica_email) > 0) {
            ...
            $this->db->delete('jun_directiva_empresas', array('id_pj' => $id_pj));
            ...
            foreach ($n_a as $row) {
                $valida = $this->db->get_where('nom_accionista_empresas', array('cedula_na' => $row['cedula_na']))->row_array();
                if (count($valida) > 0) {
                    $row['status'] = 1;
                    $this->db->where('cedula_na', $row['cedula_na']);
...
//            $this->db->insert('perfil_empresas', $p_e);
    ...
    public function salvar_ficha_pn_pj() {
        ...
        $valida = $this->db->get_where('inscripcion_pn', array('id_nom_accionista' => $form['id_nom_accionista'], 'status >' => 0))->row_array();

        if (count($valida) > 0) {
            $this->db->where('id_nom_accionista', $form['id_nom_accionista']);
            $this->db->update('inscripcion_pn', $form);
        } else {
            $this->db->insert('inscripcion_pn', $form);
            $form['id_nom_accionista'] = $this->db->insert_id();
        }

        return $this->db->get_where('inscripcion_pn', array('id_nom_accionista' => $form['id_nom_accionista'], 'status >' => 0))->row_array();
    }
